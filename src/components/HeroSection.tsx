...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    <section id="home" className="relative min-h-[calc(100vh-80px)] pt-20 flex flex-col lg:flex-row bg-[#F9FAFC] overflow-hidden">
      ...
      <div className="w-full lg:w-[60%] p-8 sm:p-12 lg:p-16 flex flex-col justify-center relative z-10">
        <div className="mb-4 inline-flex items-center gap-3 text-[#C8A84F] font-semibold text-xs tracking-[0.2em] uppercase">
          <div className="w-8 h-[1px] bg-[#C8A84F]" />
          <span>Premier Commercial Law Firm • Nigeria</span>
        </div>

        <h1 className="text-4xl sm:text-5xl lg:text-[50px] leading-[1.12] font-extrabold text-[#081826] mb-6 tracking-tight">
          Trusted Counsel. <br />
          <span className="text-[#C8A84F]">Strategic Solutions.</span> <br />
          Enduring Partnerships.
        </h1>

        <div className="mb-10 max-w-xl pl-5 border-l-2 border-[#C8A84F]/80">
        ...
        <div className="flex flex-wrap gap-4 mb-12">
          <button
            ...
            className="bg-[#081826] text-white px-8 py-4 font-bold text-xs uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-blue-900/10 hover:bg-[#143D73] transition-colors"
          >
          ...
          <button
            ...
            className="border border-[#081826] text-[#081826] px-8 py-4 font-bold text-xs uppercase tracking-wider hover:bg-[#081826] hover:text-white transition-colors flex items-center gap-2"
          >
        ...
        {/* Stats Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 border-t border-gray-200/80 pt-8">
          {stats.map((stat, idx) => (
            <div key={idx}>
              <div className="text-2xl sm:text-3xl font-extrabold text-[#081826]">
                {stat.value}
              </div>
              <div className="text-[10px] text-gray-500 uppercase tracking-widest mt-1 font-semibold">
                {stat.label}
              </div>
            </div>
          ))}
      ...
      {/* Right Column: 40% Width Dark Panel */}
      <div className="w-full lg:w-[40%] bg-[#081826] relative overflow-hidden flex flex-col justify-between min-h-[480px]">
        {/* Subtle Background Glow Angle */}
        ...
        {/* Hero Image Showcase - Fully expanded without borders */}
        <div className="w-full flex-1 relative min-h-[350px] sm:min-h-[420px] lg:min-h-[480px] overflow-hidden group">
        ...
        <div className="mt-auto p-6 sm:p-8 border-t border-[#C8A84F]/25 bg-gradient-to-t from-[#040C14] via-[#081826]/95 to-transparent backdrop-blur-md z-10">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2.5">
              <div className="p-1.5 rounded-lg bg-[#C8A84F]/15 border border-[#C8A84F]/40 text-[#C8A84F]">
                <Sparkles className="w-3.5 h-3.5" />
              </div>
              <h3 className="text-[#C8A84F] font-extrabold text-[11px] sm:text-xs tracking-[0.2em] uppercase">
                Strategic Practice Areas
              </h3>
            </div>
            <button
              onClick={onNavigateToPractices}
              className="text-[11px] font-bold text-gray-300 hover:text-[#C8A84F] transition-colors flex items-center gap-1 group"
            >
            ...
            {practiceAreasList.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  onClick={onNavigateToPractices}
                  className="group relative flex items-center gap-3 p-3 rounded-xl bg-white/[0.05] hover:bg-white/[0.12] border border-white/10 hover:border-[#C8A84F]/60 transition-all duration-300 shadow-md hover:shadow-xl hover:-translate-y-0.5 cursor-pointer overflow-hidden"
                >
                  {/* Subtle Gold Hover Glow Layer */}
                  <div className="absolute inset-0 bg-gradient-to-r from-[#C8A84F]/15 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                  
                  <div className="p-2 rounded-lg bg-[#081826] border border-white/15 group-hover:border-[#C8A84F]/50 text-[#C8A84F] group-hover:scale-110 transition-all duration-300 shrink-0 shadow-sm">
